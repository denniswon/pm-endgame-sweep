...
test.describe('Opportunities Dashboard', () => {
  ...
  test('loads and displays page header', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'PM Endgame Sweep' })).toBeVisible();
    await expect(page.getByText('Polymarket Opportunity Scanner')).toBeVisible();
  });

  test('displays opportunities section', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Opportunities' })).toBeVisible();
    await expect(page.getByText('Live opportunities updated every 30 seconds')).toBeVisible();
  });
  ...
  test('loads and displays opportunities table', async ({ page }) => {
    ...
    await expect(page.getByRole('columnheader', { name: 'Expected Payout' })).toBeVisible();
  ...
  test('navigates to market details when clicking market link', async ({ page }) => {
    // Wait for table to load
    await expect(page.getByRole('table')).toBeVisible({ timeout: 10000 });
  ...
  test('displays error state on API failure', async ({ page, context }) => {
    ...
    await expect(page.getByText(/Failed to load opportunities/)).toBeVisible({ timeout: 10000 });
  });
...
test.describe('Opportunities Table Content', () => {
  ...
  test('shows pagination information', async ({ page }) => {
    ...
    await expect(page.getByText(/Showing \d+ of \d+ opportunities/)).toBeVisible();
  });
});
